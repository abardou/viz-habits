...
export default {
	...
	data: () => ({
		...
		hoursTooltips: null,
		minutesTooltips: null
	}),
	mounted() {
		// Svg variable
		var svg = d3.select('#pickerTruc').append('svg')
			.attr('width', 960)
			.attr('height', 500);
		
		// Tooltip div
		var tooltip = d3.select('#pickerTruc').append('div').attr('class', 'hidden tooltip');
		...
		class Ring {
			...
			mouse_move(d, pathObject) {
				...
				this.tooltip.classed('hidden', false)
					.attr('style', `left: ${(mousePosition[0] + 15 + this.cx)}px;
						top: ${(mousePosition[1] - 35 + this.cy)} px`)
					.html(d);
			}
		...
		for (let i = 0; i < tt2.length; i++) tt2[i] = i+(i > 12 ? -12 : 0)+(i > 12 ? 'pm' : 'am');
		this.hoursTooltips = tt2;

		// Granularity for minutes
		const g_min = 5;
		// Tooltip for minutes
		const tt3 = [...Array(60/g_min).keys()];
		for (let i = 0; i < tt3.length; i++) tt3[i] = (g_min > 1 ? (i*g_min)+'-'+(i+1)*g_min : i);
		this.minutesTooltips = tt3;
		
		// Rings
		const r1 = new Ring(svg, tooltip, 'ring_1', 480, 250, 132, 112, '#374d7c', '#ddd', this.daysTooltips);
		const r2 = new Ring(svg, tooltip, 'ring_2', 480, 250, 106, 86, '#46edc8', '#ddd', tt2);		
		const r3 = new Ring(svg, tooltip, 'ring_3', 480, 250, 80, 60, '#fdf289', '#ddd', tt3);
